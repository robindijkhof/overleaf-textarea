...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let syncScrollCheckbox = document.getElementById('sync-scroll');
  ...
  chrome.storage.sync.get(['syncScroll'], function (result) {
    syncScrollCheckbox.checked = result.syncScroll === undefined ? true : result.syncScroll;
  });
  ...
  syncScrollCheckbox.addEventListener('change', function () {
    if (this.checked) {
      chrome.storage.sync.set({syncScroll: true}, function () {
      });
    } else {
      chrome.storage.sync.set({syncScroll: false}, function () {
      });
    }
  });
